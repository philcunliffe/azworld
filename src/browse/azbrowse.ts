...
import { getCampaignSettings, runOnboarding } from "../chat/campaign-settings";
...
function createCompleter(
  ...
  return (line: string): [string[], string] => {
    ...
    if (!navCommands.includes(cmd)) {
      ...
      if (parts.length === 1) {
        const commands = ["loc", "cd", "state", "npc", "ls", "info", "search", "gen", "mod", "rm", "help", "exit", "/talk", "/back", "/init"];
...
async function main() {
  ...
  while (true) {
    try {
      ...
      console.log(useColors ? `${RED}Error: ${e?.message || String(e)}${RESET}` : `Error: ${e?.message || String(e)}`);
    }
  }
